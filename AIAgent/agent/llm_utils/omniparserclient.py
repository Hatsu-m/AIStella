...
class OmniParserClient:
    ...
    def reformat_messages(self, response_json: dict):
        screen_info = ""
        for idx, element in enumerate(response_json.get("parsed_content_list", [])):
            element['idx'] = idx
            if element.get("type") == "text":
                screen_info += f"ID: {idx}, Text: {element.get('content')}"
            elif element.get("type") == "icon":
                screen_info += f"ID: {idx}, Icon: {element.get('content')}"
            if "bbox" in element:
                screen_info += f", bbox: {element['bbox']}"
            screen_info += "\n"
